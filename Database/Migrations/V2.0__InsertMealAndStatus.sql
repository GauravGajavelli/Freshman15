...
CREATE OR ALTER PROCEDURE insertMealAndStatus (@day Date, @meal varchar(10))
AS
BEGIN
BEGIN TRANSACTION
...
EXEC	@return_value = InsertRestaurantMealLoadStatus @RestaurantMealID = @@IDENTITY, @status = 'Unscraped';
IF (@return_value <> 0)
BEGIN
ROLLBACK TRANSACTION;
RETURN(2);
