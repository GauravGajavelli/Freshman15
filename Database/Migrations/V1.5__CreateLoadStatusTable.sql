...
CREATE TABLE RestaurantMealLoadStatus ( -- as opposed to usermeal, which will reference these as fk
  ...
  CONSTRAINT Valid_Status CHECK ([Status] IN ('Unscraped','NoMeal','Failed','Loaded')), -- So either we've tried it or not; if we've tried we're either successful or not; we can be successful knowing there is or isn't a meal
